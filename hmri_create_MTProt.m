...
if mpm_params.QA.enable && exist('fTPM','var')
    ...
    % each contrast)
    for ccon = 1:mpm_params.ncon
        %R2s=spm_read_vols(spm_vol(spm_select('FPList',calcpath,sprintf('^s.*_R2s_%sw.nii$',mpm_params.input(ccon).tag))));IRL this doesn't work properly on OsX... not sure why 
